...
const FILE_PATH = path.join(__dirname, 'multi_denom_output.json');
...
pipeline.on('data', ({ value }) => {
  for (const [address, linkedList] of Object.entries(value)) {
    if (!address || !linkedList) continue;

    // Store promises to wait later
    const p = redis
      .hset('multi_denom', address, JSON.stringify(linkedList))
      .then(() => {
        counter++;
        if (counter % 100 === 0) {
