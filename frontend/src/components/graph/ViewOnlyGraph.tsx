...
//redux
import {
  mapTransactionToNodeData,
  mapTransactionFields,
} from '@/components/graph/NodesAndEdges'
import { useDispatch } from 'react-redux' // Add useSelector to listen to Redux store

const snapGrid: SnapGrid = [20, 20]
const connectionLineStyle = { stroke: '#fff' }

import CircleNode from '@/components/graph/nodes/CircleNode'
import DefaultViewOnlyNode from '@/components/graph/nodes/viewOnly/DefaultNode'
import RedViewOnlyNode from '@/components/graph/nodes/viewOnly/RedNode'
import GrayNode from '@/components/graph/nodes/GrayNode'
import SmartBidirectionalEdge from '@/components/graph/edges/BidirectionalEdge'
import NormalDirectionalEdge from '@/components/graph/edges/NormalDirectionalEdge'
import { Transaction } from '@/types/transaction.interface'
import MultiDirectionalEdges from '@/components/graph/edges/MultiDirectionalEdge'
...
const nodeTypes: NodeTypes = {
  circle: CircleNode,
  normalAddress: DefaultViewOnlyNode,
  redAddress: RedViewOnlyNode,
...
export default function Flow({
  ...
  const initialNodes = mapTransactionToNodeData(transactions, onAddressClickAction)
  ...
  useEffect(() => {
    const newNodes = mapTransactionToNodeData(transactions, onAddressClickAction)
  ...
  return (
    <main className="grid flex-1 items-start gap-4 sm:py-0 md:gap-8 lg:grid-cols-3 xl:grid-cols-3">
      <div className="grid auto-rows-max items-start gap-4 md:gap-8 lg:col-span-3">
        <div
          className={cn(
            ' border-black rounded-lg border-dotted border-2 shadow-sm w-full h-[700px] ',
            cls?.graph
          )}
        >
          <ReactFlow
