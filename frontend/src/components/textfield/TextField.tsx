  ...
  return (
    ...
    >
      {(!!label || displayWordCount) && (
        <div
          className={cn(
            'flex items-center justify-between text-label-sm-sec text-itr-tentPri-df',
            cls?.label
          )}
        >
          <span>
      ...
      >
        {iconLeft ? (
          <span
            className={cn(
              'shrink-0',
              isDisabled ? 'text-gray-700' : 'text-itr-tentPri-df',
              ICON_SIZE_MAP[size]
            )}
          >
            {iconLeft}
          ...
          <RiInformationFill
            className={cn(
              ...
              isError
                ...
                : isWarning
                  ? 'text-itr-tone-wnSub'
                  : 'text-itr-tentPri-sub'
            )}
          />
          <span
            className={cn(
              'flex-1 text-p-sm text-left',
              isError
                ? 'text-itr-tone-dg'
                : isWarning
                  ? 'text-itr-tone-wn'
